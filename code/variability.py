...
def FindPriorRanges(xs, num_points, num_stderrs=3.0, median_flag=False):
    ...
    print(f'classical estimators {m}, {s}')
...
def PlotCoefVariation(suites):
    ...
    for label, suite in suites.iteritems():
        ...
        print(f'CV posterior mean, {pmf.Mean()}')
    ...
    thinkplot.Save(root='variability_cv',
                ...
                ylabel='Probability')
    fb = thinkbayes.PmfProbGreater(pmfs["female"], pmfs["male"])
    mb = thinkbayes.PmfProbGreater(pmfs['male'], pmfs['female'])
    print(f'female bigger, {fb}')
    print(f'male bigger,{mb}')
...
def Summarize(xs):
    ...
    xs.sort()
    print(f'smallest, {xs[:10]}')
    print(f'largest, {xs[-10:]}')
